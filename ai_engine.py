...
* שמור על ניסוח מותאם לשוק הישראלי."""
    ...
    if max_pages >= 2:
        content_limits_block = """מגבלות תוכן מחייבות לשני עמודים - חובה לקיים בדיוק:
* תקציר מקצועי: עד 4 משפטים בלבד, לא יותר מ-80 מילים בסך הכל.
* ניסיון תעסוקתי: עד 6 תפקידים בלבד. לכל תפקיד: עד 5 נקודות תמציתיות לכל היותר, כל נקודה עד 15 מילים.
* השכלה: עד 5 ערכים בלבד.
* מיומנויות: עד 12 מיומנויות טכניות ועד 8 מיומנויות רכות.
* קורסים, הסמכות ופרויקטים: עד 6 פריטים בלבד.

סדר מחייב של קורות החיים:
1. פרטים אישיים - שם מלא, טלפון, אימייל, לינקדאין, עיר מגורים אופציונלית.
2. תקציר מקצועי - עד 4 משפטים (לא יותר מ-80 מילים): זהות מקצועית, תחום התמחות, ערך מקצועי מרכזי.
3. ניסיון תעסוקתי - עד 6 תפקידים אחרונים בלבד. לכל תפקיד: שנים → שם חברה → תפקיד → עד 5 נקודות תמציתיות בלבד, כל נקודה עד 15 מילים.
4. שירות צבאי - אם מופיע במקור.
5. השכלה - שנים → מוסד → תואר → תחום לימוד. עד 5 ערכים.
6. מיומנויות - עד 12 מיומנויות טכניות ועד 8 מיומנויות רכות.
7. קורסים, הסמכות ופרויקטים - אם מופיעים במקור, עד 6 פריטים בלבד."""
    else:
        content_limits_block = """מגבלות תוכן מחייבות לעמוד אחד - חובה לקיים בדיוק:
* תקציר מקצועי: עד 3 משפטים בלבד, לא יותר מ-50 מילים בסך הכל.
* ניסיון תעסוקתי: עד 4 תפקידים בלבד. לכל תפקיד: בדיוק 3 נקודות תמציתיות לכל היותר, כל נקודה עד 12 מילים.
* השכלה: עד 3 ערכים בלבד.
* מיומנויות: עד 8 מיומנויות טכניות ועד 5 מיומנויות רכות.
* קורסים, הסמכות ופרויקטים: עד 3 פריטים בלבד.

סדר מחייב של קורות החיים:
1. פרטים אישיים - שם מלא, טלפון, אימייל, לינקדאין, עיר מגורים אופציונלית.
2. תקציר מקצועי - עד 3 משפטים (לא יותר מ-50 מילים): זהות מקצועית, תחום התמחות, ערך מקצועי מרכזי.
3. ניסיון תעסוקתי - עד 4 תפקידים אחרונים בלבד. לכל תפקיד: שנים → שם חברה → תפקיד → עד 3 נקודות תמציתיות בלבד, כל נקודה עד 12 מילים.
4. שירות צבאי - אם מופיע במקור.
5. השכלה - שנים → מוסד → תואר → תחום לימוד. עד 3 ערכים.
6. מיומנויות - עד 8 מיומנויות טכניות ועד 5 מיומנויות רכות.
7. קורסים, הסמכות ופרויקטים - אם מופיעים במקור, עד 3 פריטים בלבד."""

    _pages_tech_note = (
        f"שמור על תמציתיות - קורות חיים חייבים להיכנס ל-{max_pages} עמודות לכל היותר"
        if max_pages >= 2 else
...
* שמור על ניסוח מותאם לשוק הישראלי."""
    ...
    if max_pages >= 2:
        _build_content_limits_block = """מגבלות תוכן מחייבות לשני עמודים - חובה לקיים בדיוק:
* תקציר מקצועי: עד 4 משפטים בלבד, לא יותר מ-80 מילים בסך הכל.
* ניסיון תעסוקתי: עד 6 תפקידים בלבד. לכל תפקיד: עד 5 נקודות תמציתיות לכל היותר, כל נקודה עד 15 מילים.
* השכלה: עד 5 ערכים בלבד.
* מיומנויות: עד 12 מיומנויות טכניות ועד 8 מיומנויות רכות.
...
5. השכלה - שנים → מוסד → תואר → תחום לימוד. עד 5 ערכים.
6. מיומנויות - עד 12 מיומנויות טכניות ועד 8 מיומנויות רכות.
7. קורסים, הסמכות ופרויקטים - אם מופיעים בנתונים, עד 6 פריטים בלבד."""
    else:
        _build_content_limits_block = """מגבלות תוכן מחייבות לעמוד אחד - חובה לקיים בדיוק:
* תקציר מקצועי: עד 3 משפטים בלבד, לא יותר מ-50 מילים בסך הכל.
* ניסיון תעסוקתי: עד 4 תפקידים בלבד. לכל תפקיד: בדיוק 3 נקודות תמציתיות לכל היותר, כל נקודה עד 12 מילים.
* השכלה: עד 3 ערכים בלבד.
* מיומנויות: עד 8 מיומנויות טכניות ועד 5 מיומנויות רכות.
...
5. השכלה - שנים → מוסד → תואר → תחום לימוד. עד 3 ערכים.
6. מיומנויות - עד 8 מיומנויות טכניות ועד 5 מיומנויות רכות.
7. קורסים, הסמכות ופרויקטים - אם מופיעים בנתונים, עד 3 פריטים בלבד."""
...
כללים טכניים:
- שמור על תמציתיות - קורות חיים חייבים להיכנס ל-{max_pages} עמוד{"ות" if max_pages >= 2 else ""} לכל היותר
- אל תוסיף כותרות כמו "קורות חיים" - רק התוכן עצמו
