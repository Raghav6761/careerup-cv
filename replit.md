...
7. **Per-section consultation chat (Build flow)**: Each content section in the build-from-scratch form has a "💬 התייעץ" button that expands the section card inline and reveals a chat panel with an OpenAI GPT-5 career advisor focused on that specific section (target/personal/summary/experience/education/skills/military/volunteering/projects/additional). Only one section can be open at a time; clicking the same section's button again (label switches to "✕ סגור התייעצות") collapses the panel. Chat history kept per-section in `st.session_state.consultation_chats`, persisted to localStorage via `persistence.py` (last 30 messages per section). Backend: `section_consultation_reply()` and `section_consultation_greeting()` in `ai_engine.py` (with `_SECTION_LABELS` and `_SECTION_GUIDANCE` dicts). UI: `_render_consult_button()` (toggle) and `_render_inline_consultation()` (chat panel rendered at the bottom of the section's `st.container`) helpers in `app.py`. Open-section state tracked in `st.session_state.open_consultation`.
